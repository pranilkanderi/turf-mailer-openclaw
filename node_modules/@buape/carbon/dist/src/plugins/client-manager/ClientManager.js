...
export class ClientManager {
    ...
    constructor(options, handlers, plugins) {
        ...
        this.setupRoutes();
        this.startupPromise = this.initializeClients(options.initialSetupOptions, options.startup);
    ...
    async initializeClients(initialSetupOptions, startupOptions) {
        ...
        let nextIndex = 0;
        const results = [];
        const workers = Array.from({ length: Math.min(concurrency, applications.length) }, async () => {
            while (true) {
                if (shouldStop)
                    return;
        ...
        if (failFast && failures > 0) {
            throw new Error(`ClientManager startup failed with ${failures} error(s): ${results
