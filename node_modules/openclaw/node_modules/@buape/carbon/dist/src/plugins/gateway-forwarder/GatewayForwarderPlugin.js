...
export class GatewayForwarderPlugin extends GatewayPlugin {
    ...
    options;
    privateKey;
    guildAvailabilityCache = new Map();
    deliveryPolicy;
    deliveryExecutor;
    forwardFetch;
    deliveryMetrics = {
    ...
    constructor(options) {
        ...
        this.deliveryPolicy = {
            ...defaults,
            ...options.delivery,
            retryStatusCodes: options.delivery?.retryStatusCodes ?? defaults.retryStatusCodes
        };
        assertPositiveInteger("delivery.maxInFlight", this.deliveryPolicy.maxInFlight);
        this.forwardFetch = options.fetch;
        this.deliveryExecutor = createBoundedExecutor({
            ...
            try {
                ...
                if (payload.t && payload.d) {
                    ...
                    if (gatewayType === "GUILD_CREATE") {
                        ...
                        const cachedAvailability = this.guildAvailabilityCache.get(guildCreateData.id);
    ...
    enqueueDelivery(task) {
        if (this.deliveryExecutor.getQueueDepth() >= this.deliveryPolicy.maxQueueSize) {
    ...
    async deliver(task) {
        for (let attempt = task.attempt; attempt <= this.deliveryPolicy.maxRetries; attempt += 1) {
    ...
    async sendWithTimeout(task, attempt) {
        ...
        try {
            ...
            return {
                ok: false,
                retryable: this.deliveryPolicy.retryStatusCodes.includes(response.status),
                reason
            };
        }
        catch (error) {
