import { ApplicationCommandType, ApplicationWebhookType, InteractionResponseType, InteractionType, Routes } from "discord-api-types/v10";
...
export class Client {
    ...
    getRuntimeMetrics() {
        const forwarderPlugin = this.getPlugin("gateway-forwarder");
        return {
            request: this.rest.getSchedulerMetrics(),
            events: this.eventHandler.getMetrics(),
            forwarder: typeof forwarderPlugin?.getDeliveryMetrics === "function"
        ...
        if (this.options.devGuilds && this.options.devGuilds.length > 0) {
            ...
            if (primaryEntryPointCommands.length > 0) {
                const deployed = (await this.rest.put(Routes.applicationCommands(this.options.clientId), {
                    body: primaryEntryPointCommands.map((command) => command.serialize())
                }));
                this.updateCommandIdsFromDeployment(deployed);
    ...
    async reconcileGlobalCommands(commands) {
        ...
        const normalizeCommandDefinitionValue = (value, path = []) => {
            ...
            if (value && typeof value === "object") {
                const normalizedEntries = Object.entries(value).flatMap(([key, entry]) => {
                    ...
                    if ((key === "required" || key === "autocomplete") &&
                        entry === false) {
                        return [];
                    }
        ...
        const normalizeLiveCommandDefinition = (command) => {
            const body = Object.fromEntries(Object.entries(command).filter(([key]) => !discordCommandResponseOnlyFields.has(key)));
        ...
        const liveByKey = new Map(liveCommands.map((command) => [
        ...
        for (const desired of desiredCommands) {
            ...
            if (!existing)
                continue;
            if (JSON.stringify(normalizeLiveCommandDefinition(existing)) ===
