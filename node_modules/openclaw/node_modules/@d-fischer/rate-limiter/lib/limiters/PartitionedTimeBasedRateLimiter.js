...
const RateLimiterDestroyedError_1 = require("../errors/RateLimiterDestroyedError");
class PartitionedTimeBasedRateLimiter {
    ...
    async request(req, options) {
        return await new Promise((resolve, reject) => {
            var _a, _b;
            if (this._destroyed) {
                reject(new RateLimiterDestroyedError_1.RateLimiterDestroyedError('Rate limiter was destroyed'));
                return;
            }
            const reqSpec = {
                req,
    ...
    destroy() {
        ...
        for (const queue of this._partitionedQueue.values()) {
            for (const req of queue) {
                req.reject(new RateLimiterDestroyedError_1.RateLimiterDestroyedError('Rate limiter was destroyed'));
            }
        }
        this._partitionedQueue.clear();
    }
